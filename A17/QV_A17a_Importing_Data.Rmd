...
variable.names(imdbMessy)
table(imdbMessy$color)

imdbMessy$color<-as.factor(imdbMessy$color)

#the varible color has values that aren't quite tidy, there are some values that are N/A, COLOR,Color, Black and White, B&W - there's some information to be condensed here

levels(imdbMessy$color) <- list(Color=c("Color","COLOR"), `Black and White`=c("Black and White","B&W"), `N/A`=c("N/A"))
```
...
*A lot of these N/A's seem to pop up for movies that aren't very popular, like for example the `gross` column has a lot of N/A's for movies I've never heard of, and same with `director_name`.*
...
[^1]: This is dangerous unless you know what you are doing.
...
#seems like all the titles with `actor_1_facebook_likes` N/A also have most other columns N/A. This could be because there just isn't much data on these movies because of how unpopular they are.

imdbCleaned1 <- imdbMessy |>
  drop_na(`actor_1_facebook_likes`)

imdbCleaned2 <- imdbMessy |>
...
```{r, fig.width=10}
library(readr)
project_sunroof_census_tract <- read_csv("~/Documents/GitHub/solar-incentive-adoption/project-sunroof-census_tract.csv", 
    col_types = cols(state_name = col_factor(levels = c("Alabama", 
        ...
        "Wisconsin", "Wyoming"))))
View(project_sunroof_census_tract)

project_sunroof_summary <- project_sunroof_census_tract |>
  group_by(state_name) |>
  summarise(number_of_panels_n = sum(number_of_panels_n)) |>
  drop_na()

colnames(project_sunroof_summary) <- c("state","num_panels")
...
plot_usmap(data = project_sunroof_summary, values = "num_panels", color = "black") + 
  scale_fill_continuous(
    low = "grey", high = "red", name = "Number of Panels") + 
  theme(legend.position = "right") +
  labs(title = "Solar Panel Count by State")
```
